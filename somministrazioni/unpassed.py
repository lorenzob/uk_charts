...
for dose in dosi:
	for filtra_regione in ["ITA"] + list(all_data['area'].unique()):
		...
		num_casi = casi.groupby(['fascia_anagrafica']).sum().reset_index()

		with pd.option_context('display.max_columns', None,):
			print(num_casi)

		casi_by_date = num_casi[['fascia_anagrafica', 'prima_dose', 'seconda_dose', 'pregressa_infezione', 'dose_addizionale_booster']]
