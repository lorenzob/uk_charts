...
for reg_id in range(1, 23):	
	...
	num_casi["rapporto tamponi m/ag"] = 100 * num_casi["nuovi_tamponi_ag"] / (num_casi["nuovi_tamponi_m"] + num_casi["nuovi_tamponi_ag"])
	num_casi["rapporto tamponi m/ag MA"] = num_casi["rapporto tamponi m/ag"].rolling(window=7).mean() 


	#num_casi.plot(kind='line', x='data', y='rapporto m/ag MA', color='firebrick', linewidth=1, ax=ax2, label="rapporto % positivi")
	num_casi.plot(kind='line', x='data', y='rapporto tamponi m/ag MA', color='firebrick', linewidth=1, ax=ax2, label="% tamponi rapidi sul totale")
	...
	plt.title('Rapporto tra tamponi rapidi e molecolari', fontsize=6)
	...
	plt.savefig(f"charts/rapporto_test_{regione}.png", dpi=200) 
