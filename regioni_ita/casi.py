...
for reg_id in range(0, 23):	
	...
	num_casi = num_casi[['data', 'nuovi_tamponi_m', 'nuovi_tamponi_ag', 'nuovi_positivi_m', 'nuovi_positivi_ag']]
	...
	num_casi.set_index('data')
	#num_casi = num_casi.rolling(axis=0, window=7).mean()
	print(num_casi)

	num_casi.plot(kind='line', y='nuovi_positivi_m', color='#0343df', linewidth=1, ax=ax2, label="molecolari positivi")
	num_casi.plot(kind='line', y='nuovi_positivi_ag', color='firebrick', linewidth=1, ax=ax2, label="antigenici positivi")
	...
	plt.text(345, 0, f"({str(all_data['data'].iloc[-1])[:10]})", fontsize = 4)

	plt.tight_layout()

	plt.savefig(f"charts/casi_{regione}.png", dpi=200) 
